        ...
        public ActionResult Index()
        {
            ...
            foreach (var order in ordersYesterday)
            {
                if (order.OrderTransports != null && order.OrderTransports.Count > 0)
                {
                    ...
                    if (order.DateEnd <= dateEndDuration)
                    {
                        ...
                        YesterdayProfit += payment.CashPayment + payment.CardPayment;
                        YesterdayCashPaymentProfit += payment.CashPayment;
                        YesterdayCardPaymentProfit += payment.CardPayment;
            ...
            foreach (var order in ordersToday)
            {
                if (order.OrderTransports != null && order.OrderTransports.Count >0)
                {
                    ...
                    {
                        var payment = order.Payment;
                        TodayProfit +=  payment.CashPayment +  payment.CardPayment;
                        TodayCashPaymentProfit += payment.CashPayment;
                        TodayCardPaymentProfit +=  payment.CardPayment;
